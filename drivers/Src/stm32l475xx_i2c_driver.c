...
static void I2C_RestartCondition(I2C_RegDef_t *pI2Cx, uint8_t RestartdEnorDi);
 ...
 */
static void  I2C_TransferDirWorRMasterM(I2C_RegDef_t *pI2Cx, uint8_t RorWTransfer){
	if(RorWTransfer == I2C_WRITE_TRANSFER_MM){  // Master requests a write transfer.   (master mode)
...
static void I2C_AutoendCondition(I2C_RegDef_t *pI2Cx, uint8_t AutoendEnorDi){
	if(AutoendEnorDi == I2C_AUTOEND_DISABLE_MM){  // Master requests a write transfer.   (master mode)
		pI2Cx -> CR2 &= ~(1 << I2C_CR2_AUTOEND); //1: Automatic end mode: a STOP condition is automatically sent when NBYTES data are transferred.
	}else if(AutoendEnorDi == I2C_AUTOEND_ENABLE_MM){
...
void Timing_Settings(I2C_Handle_t *pI2CHandle){
   ...
   //pI2CHandle ->pI2Cx ->TIMINGR |= (0x13 << I2C_TIMINGR_SCLL);
	...
	pI2CHandle ->pI2Cx ->TIMINGR |= (0x3e << I2C_TIMINGR_SCLH);
	...
	pI2CHandle ->pI2Cx ->TIMINGR |= (0x2 << I2C_TIMINGR_PRESC);
}
/*
 ...
 * Data Send
 */
/*void I2C_MasterSendData(I2C_Handle_t *pI2CHandle, uint8_t *pTxBuffer, uint8_t Len, uint8_t SlaveAddr,uint8_t RorWTransfer){
...
void I2C_MasterSendData(I2C_Handle_t *pI2CHandle, uint8_t *pTxBuffer, uint8_t Len, uint8_t SlaveAddr,uint8_t RorWTransfer, uint8_t AutoendEnorDi, uint8_t RestartdEnorDi){
	...
	//or greater than 255 bytes, NBYTES[7:0] must initially be filled with 0xFF.
     ...
     I2C_RestartCondition(pI2CHandle -> pI2Cx, RestartdEnorDi);
     //I2C_NBytestoControl(pI2CHandle -> pI2Cx);
     // Start generation with not Bus busy
     //while((I2C_GetFlagStatus(pI2CHandle -> pI2Cx, I2C_FLAG_BUSY)));
    // Start generation
	...
	while(Len > 0 ){
		if(Len <= 255 ){
			...
			//If the total number of data bytes to be sent is greater than 255, reload mode
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle){
	...
	if(tempTCR && tempFlag){
		...
		if(pI2CHandle -> TxRxState == I2C_BUSY_IN_TX){
			//if(pI2CHandle -> TxLen > 255){
					if(((pI2CHandle -> pI2Cx -> CR2 & (1 << I2C_CR2_AUTOEND)) == DISABLE) && (pI2CHandle -> pI2Cx -> CR2 & (1 << I2C_CR2_RELOAD) == ENABLE)){
							// In this mode, software action is expected once the number of bytes programmed in the NBYTES[7:0] bit field is transferred; the TC flag is set and an interrupt is generated if the TCIE bit is set
							I2C_GenerateStartCondition(pI2CHandle -> pI2Cx);
	...
	if(tempTCR && tempFlag){
		...
		if(pI2CHandle -> TxRxState == I2C_BUSY_IN_TX){
			//if(pI2CHandle -> TxLen == 0){
			// TCR flag is set
				if((pI2CHandle -> pI2Cx -> CR2 & (1 << I2C_CR2_AUTOEND) == ENABLE) && (pI2CHandle -> pI2Cx -> CR2 & (1 << I2C_CR2_RELOAD) == DISABLE)){
